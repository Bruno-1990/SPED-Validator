...
**Versão:** 5.0.0-final
**Escopo:** EFD ICMS/IPI exclusivamente — PIS/COFINS aparecem apenas como reflexo tributário. EFD Contribuições fora do escopo.
...
class BeneficioEngine:
    ...
    CSOSN_TO_CST: dict[str, str] = {
        ...
        "203": "10",   # Isenção do ICMS para faixa de receita bruta com ST
        "300": "41",   # Imune
...
### 4.3 Tratamento de Benefícios Simultâneos
...
|---|---|---|
| Soma C170 fecha com C100 | C100 × SUM(C170) | `CRUZAMENTO_DIVERGENTE` |
| Todos os C170 têm C190 | C170 × C190 (por CST+CFOP+ALIQ) | `C190_DIVERGE_C170` |
| C190 sem C170 lastro | C190 × C170 | `C190_COMBINACAO_INCOMPATIVEL` |
| Item sem C100 pai | C170 órfão | `C170_ORFAO` |

#### Trilha 2: Consolidação → Apuração

| Verificação | Registros | Fórmula | Error Type |
|---|---|---|---|
| C190 alimenta E110 débitos | C190 saídas → E110.VL_TOT_DEBITOS | `SUM(C190.VL_ICMS_TOT) CFOP 5/6/7 ≈ VL_TOT_DEBITOS` | `APURACAO_DEBITO_DIVERGENTE` |
| C190 alimenta E110 créditos | C190 entradas → E110.VL_TOT_CREDITOS | `SUM(C190.VL_ICMS_TOT) CFOP 1/2/3 ≈ VL_TOT_CREDITOS` | `APURACAO_CREDITO_DIVERGENTE` |
| Saldo E110 fecha | E110 | `VL_SLD_APURADO = VL_TOT_DEBITOS − VL_TOT_CREDITOS + ajustes` | `APURACAO_SALDO_DIVERGENTE` |
| E111 com código de ajuste válido | E111 × `codigos_ajuste_uf.yaml` | Código válido para UF | `CODIGO_AJUSTE_INVALIDO` |

#### Trilha 3: Benefício → Ajuste → Apuração
...
| Error Type | Severidade | Certeza | Descrição |
|---|---|---|---|
| `APURACAO_DEBITO_DIVERGENTE` | error | objetivo | SUM(C190 saídas) ≠ E110.VL_TOT_DEBITOS |
...
### 14.4 Persona da IA para ICMS/IPI

```
Você é um assistente fiscal especializado em EFD ICMS/IPI.
...
- Formato: 3 parágrafos: O QUE está errado · POR QUE é um problema · COMO corrigir
- Sempre mencione o benefício fiscal quando relevante (COMPETE-ES, FUNDAP, INVEST-ES)
- Sugestão não vinculante — decisão é do contador responsável
...
```
COBERTURA_REGRAS = regras_executadas / regras_ativas
  (regras puladas por tabela ausente reduzem esta métrica)

COBERTURA_DOCUMENTAL = nfe_com_xml / total_nfe_sped
  (apenas Trilha B)

COBERTURA_ITENS = itens_cruzados / total_itens_sped
  (apenas Trilha B)

NOTA_COBERTURA = (COBERTURA_REGRAS × 0.5) + (COBERTURA_DOCUMENTAL × 0.3) + (COBERTURA_ITENS × 0.2)
```
